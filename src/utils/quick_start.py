...
def quick_start(model, dataset, config_dict, args, save_model=True, mg=False):
    # merge config dict
    ...
    for hyper_tuple in combinators:
        ...
        model = get_model(config['model'])(config, train_data, args).to(config['device'])
        ...
        # wandb.log({f'best_test_{key}': value for key, value in hyper_ret[best_test_idx][2].items()})
